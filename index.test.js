...
const rooms= [{
  ...
  describe('Comprueba que el precio está en céntimos y los errores de discount', () => {
    ...
    test('Si el precio de room es mayor que 100 o menor que 0 devuelve un error', () => {
        expect(() =>  new Room(rooms[0].name, rooms[0].rate, 101).discountError()).toThrowError("El número de descuento tiene que estar comprendido entre 0 y 100")
    })

    test('Si el precio de room está entre 0 y que 100 devuelve el descuento', () => {
        expect(new Room(rooms[0].name, rooms[0].rate, rooms[0].discount).discountError()).toBe(30)
    })

    test('Si el precio de booking está entre 0 y que 100 devuelve el descuento', () => {
        expect(new Booking(bookings[0].name, bookings[0].email, bookings[0].check_in, bookings[0].check_out, bookings[0].discount).discountError()).toBe(10)
    })

    test('Si el precio de booking es mayor que 100 o menor que 0 devuelve un error', () => {
        expect(() =>  new Booking(bookings[0].name, bookings[0].email, bookings[0].check_in, bookings[0].check_out, 101).discountError()).toThrowError("El número de descuento tiene que estar comprendido entre 0 y 100")
    })
...
describe("Comprueba el porcentaje de ocupación", () => {
    ...
    test('totalOcuppancyPorcentaje() devuelve el porcentaje de días de ocupación de las habitaciones', () => {
        ...
        expect(Room.totalOccupancyPorcentaje(room, new Date('2023-12-1'), new Date('2023-12-31'))).toBe("33.33")
    })
